...
if [ -n "$SERVER_FILES_DIR" ] && [ -f "$SERVER_FILES_DIR/docker-compose.yml" ]; then
    scp -i "$SSH_KEY" -o "StrictHostKeyChecking=no" "$TECH_DIR/.env.production" "${SSH_USER}@${SERVER_IP}:${DEPLOY_PATH}/domains/eduviskar.com/" || write_warn "Failed SCP of .env.production"
